 ...
 *
 * This will become clearer in the function bp_wdf_screen_one() when you want to load
 * a template file.
 */
function bp_wdf_load_template_filter( $found_template, $templates ) {
	global $bp;
	...
	return apply_filters( 'bp_wdf_load_template_filter', $found_template );
}
add_filter( 'bp_located_template', 'bp_wdf_load_template_filter', 10, 2 );
...
function bp_wdf_accept_terms() {
	 ...
	 */
	check_admin_referer( 'bp_wdf_accept_terms' );
	...
	bp_wdf_record_activity( array(
		'type' => 'accepted_terms',
		'action' => apply_filters( 'bp_wdf_accepted_terms_activity_action', sprintf( __( '%s akzeptierte die Geschäftsbedingungen!', 'wdf' ), $user_link ), $user_link ),
	) );

	/* See bp_wdf_reject_terms() for an explanation of deleting activity items */
	...
	do_action( 'bp_wdf_accept_terms', $bp->loggedin_user->id );
 ...
 * Records an activity stream item for the user.
 */
function bp_wdf_reject_terms() {
	global $bp;

	check_admin_referer( 'bp_wdf_reject_terms' );
	...
	bp_wdf_record_activity( array(
		'type' => 'rejected_terms',
		'action' => apply_filters( 'bp_wdf_rejected_terms_activity_action', sprintf( __( '% s lehnte die Geschäftsbedingungen ab.', 'wdf' ), $user_link ), $user_link ),
	) );
	...
	do_action( 'bp_wdf_reject_terms', $bp->loggedin_user->id );

	return true;
}

/**
 * bp_wdf_send_donation()
 *
 ...
 */
function bp_wdf_send_donation( $to_user_id, $from_user_id ) {
	global $bp;

	check_admin_referer( 'bp_wdf_send_donation' );

	/**
	 * We'll store donations as usermeta, so we don't actually need
	 * to do any database querying. If we did, and we were storing them
	 * in a custom DB table, we'd want to reference a function in
	 * bp-wdf-classes.php that would run the SQL query.
	 */
	delete_user_meta( $to_user_id, 'donations' );
	/* Get existing fives */
	$existing_fives = maybe_unserialize( get_user_meta( $to_user_id, 'donations', true ) );

	/* Check to see if the user has already high-fived. That's okay, but lets not
	 * store duplicate donations in the database. What's the point, right?
	 */
	if ( !in_array( $from_user_id, (array)$existing_fives ) ) {
		...
		update_user_meta( $to_user_id, 'donations', serialize( $existing_fives ) );

		// Let's also record it in our custom database tables
		$db_args = array(
			'recipient_id'  => (int)$to_user_id,
			'funder_id' => (int)$from_user_id
		);

		$donation = new BP_WDF_Donation( $db_args );
		$donation->save();
	}
	 ...
	 * this notification in bp_wdf_format_notifications() using the 'new_donation' action.
	 */
	bp_core_add_notification( $from_user_id, $to_user_id, $bp->wdf->slug, 'new_donation' );

	/* Now record the new 'new_donation' activity item */
	$to_user_link = bp_core_get_userlink( $to_user_id );
	$from_user_link = bp_core_get_userlink( $from_user_id );

	bp_wdf_record_activity( array(
		'type' => 'rejected_terms',
		'action' => apply_filters( 'bp_wdf_new_donation_activity_action', sprintf( __( '%s unterstützt %s!', 'wdf' ), $from_user_link, $to_user_link ), $from_user_link, $to_user_link ),
		'item_id' => $to_user_id,
	) );

	/* We'll use this do_action call to send the email notification. See bp-wdf-notifications.php */
	do_action( 'bp_wdf_send_donation', $to_user_id, $from_user_id );

	return true;
}

/**
 * bp_wdf_get_donations_for_user()
 *
 * Returns an array of user ID's for users who have high fived the user passed to the function.
 */
function bp_wdf_get_donations_for_user( $user_id ) {
	global $bp;

	if ( !$user_id )
		return false;

	return maybe_unserialize( get_user_meta( $user_id, 'donations', true ) );
}
 ...
 */
function bp_wdf_remove_data( $user_id ) {
	...
	delete_user_meta( $user_id, 'bp_wdf_some_setting' );

	do_action( 'bp_wdf_remove_data', $user_id );
}
add_action( 'wpmu_delete_user', 'bp_wdf_remove_data', 1 );
add_action( 'delete_user', 'bp_wdf_remove_data', 1 );
 ...
 *
 * In parts of this wdf component you will see calls to wp_cache_get() often in template tags
