...
function bp_wdf_screen_notification_settings() {
	...
	<table class="notification-settings" id="bp-wdf-notification-settings">
		...
		<tr>
			<th class="icon"></th>
			<th class="title"><?php _e( 'Spenden', 'wdf' ) ?></th>
			<th class="yes"><?php _e( 'Ja', 'wdf' ) ?></th>
			<th class="no"><?php _e( 'Nein', 'wdf' )?></th>
		</tr>
		</thead>
	...
	switch ( $action ) {
		case 'new_donation':
			...
			if ( (int)$total_items > 1 ) {
				return apply_filters( 'bp_wdf_multiple_new_donation_notification', '<a href="' . $bp->loggedin_user->domain . $bp->wdf->slug . '/screen-one/" title="' . __('Mehrfachspenden', 'wdf') . '">' . sprintf( __( '%d neue High-Fives, Multi-Five!', 'wdf' ), (int)$total_items ) . '</a>', $total_items );
			} else {
				$user_fullname = bp_core_get_user_displayname( $item_id, false );
				$user_url = bp_core_get_user_domain( $item_id );
				return apply_filters( 'bp_wdf_single_new_donation_notification', '<a href="' . $user_url . '?new" title="' . $user_fullname .'\'s profile">' . sprintf( __('%s hat dir einen High-Five geschickt!', 'wdf'), $user_fullname ) . '</a>', $user_fullname );
			}
		break;
	}
...
function bp_wdf_send_donation_notification( $to_user_id, $from_user_id ) {
	...
	$subject = '[' . get_blog_option( 1, 'blogname' ) . '] ' . sprintf( __( '%s High-Fived Dich!', 'wdf' ), stripslashes($sender_name) );

	$message = sprintf( __(
'%s hat dir einen High-Five geschickt! Warum nicht einen zurückschicken?

Profil von %s anzeigen: %s

Sende %s einen High-Five: %s

---------------------
', 'wdf' ), $sender_name, $sender_name, $sender_profile_link, $sender_name, $sender_donation_link );

	$message .= sprintf( __( 'Um diese Benachrichtigungen zu deaktivieren, melde Dich bitte an und gehe zu: %s', 'wdf' ), $reciever_settings_link );
