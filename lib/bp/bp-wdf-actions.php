...
function bp_wdf_donation_save() {
	...
	if ( bp_is_wdf_component() && bp_is_current_action( 'screen-one' ) && bp_is_action_variable( 'send-donation', 0 ) ) {
		...
		if ( bp_is_my_profile() ) {
			...
			bp_core_add_message( __( 'Du kannst Dich nicht selbst unterstützen:', 'wdf' ), 'error' );
		} else {
			if ( bp_wdf_send_donation( bp_displayed_user_id(), bp_loggedin_user_id() ) )
				bp_core_add_message( __( 'Unterstützung gesendet!', 'wdf' ) );
			else
				bp_core_add_message( __( 'Unterstützung konnte nicht gesendet werden.', 'wdf'), 'error' );
		}
