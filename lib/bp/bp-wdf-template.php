...
function bp_wdf_pagination_count() {
	...
	function bp_wdf_get_pagination_count() {
		...
		$pagination_count = sprintf( __( 'Zeige Seite %1$s von %2$s', 'wdf' ), $items_template->query->query_vars['paged'], $items_template->query->max_num_pages );
...
function bp_wdf_donation_title() {
	...
	function bp_wdf_get_donation_title() {
		...
		$title 		 = sprintf( __( '%1$s gab %2$s einen High-Five!', 'wdf' ), $funder_link, $recipient_link );
