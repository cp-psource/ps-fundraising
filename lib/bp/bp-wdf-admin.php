...
function bp_wdf_add_admin_menu() {
	...
	add_submenu_page( 'bp-components', __( 'Fundraising Admin', 'wdf' ), __( 'Fundraising Admin', 'wdf' ), 'manage_options', 'bp-wdf-settings', 'bp_wdf_admin' );
}
...
?>
	<div class="wrap">
		...
		<?php if ( isset($updated) ) : ?><?php echo "<div id='message' class='updated fade'><p>" . __('Einstellungen aktualisiert.', 'wdf') . "</p></div>" ?><?php endif; ?>
		...
		<form action="<?php echo site_url() . '/wp-admin/admin.php?page=bp-wdf-settings' ?>" name="wdf-settings-form" id="wdf-settings-form" method="post">
			...
			<table class="form-table">
				<tr valign="top">
					<th scope="row"><label for="target_uri"><?php _e( 'Option Eins', 'wdf' ) ?></label></th>
					<td>
						<input name="wdf-setting-one" type="text" id="wdf-setting-one" value="<?php echo esc_attr( $setting_one ); ?>" size="60" />
					</td>
				</tr>
					<th scope="row"><label for="target_uri"><?php _e( 'Option Zwei', 'wdf' ) ?></label></th>
					<td>
			...
			<p class="submit">
				<input type="submit" name="submit" value="<?php _e( 'Einstellungen speichern', 'wdf' ) ?>"/>
			</p>
