		...
		function print_footer_list() {
			...
			foreach ( $this->page_pointers as $pointer_id => $settings) {
				...
				if ( $count >= count($this->page_pointers) && isset($this->registered_pointers[$pointer_id+1]) ) {
					...
					$next_name = __('Beenden', $this->textdomain);
					$next_title = sprintf(__('Beenden %s', $this->textdomain), $this->tutorial_name);
					$last_step = true;
				}

				$prev_link = '';
				$prev_pointer = '';
				$prev_name = __('&laquo; Vorherige', $this->textdomain);
				...
				$close_name = __('Beenden', $this->textdomain);
				$close_title = sprintf(__('Beenden %s', $this->textdomain), $this->tutorial_name);
				?>
				...
				options<?php echo $pointer_id; ?> = $.extend( options<?php echo $pointer_id; ?>, {
					...
					buttons: function( event, t ) {
						var $buttons = $(
							...
							'<span class="tut-step"><?php printf( __('%s: Schritt %d von %d', $this->textdomain), $this->tutorial_name, $pointer_id+1, count($this->registered_pointers) ); ?></span>' +
							<?php } ?>
