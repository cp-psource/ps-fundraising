...
class WDF_Fundraisers_List extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( false, sprintf(__('%s Liste','wdf'),esc_attr($settings['funder_labels']['plural_name'])), array(
			'description' =>  sprintf(__('Wähle eine Liste einfacher %s Links aus, die Du anzeigen möchtest.','wdf'),esc_attr($settings['funder_labels']['plural_name']))
		) );
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'description' ); ?>"><?php echo __('Beschreibung','wdf') ?></label><br />
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'limit' ); ?>"><?php echo __('Maximale Anzahl von Spendenaktionen, die angezeigt werden sollen','wdf') ?></label><br />
		...
		$query = get_posts($query);
		if(count($query) > 0) {
			echo '<p><span>'.sprintf(__('Optional: Wähle %s zur Anzeige aus','wdf'), esc_attr($settings['funder_labels']['plural_name'])).'</span><br/>';
			foreach($query as $funder) : ?>
