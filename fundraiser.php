...
class WDF {
	...
	function create_funder_labels() {
		...
		$funder_labels = array(
			...
			'add_new' => sprintf(__('Neue %s','wdf'), $single),
			'add_new_item' => sprintf(__('Neue %s hinzufügen','wdf'), $single),
			'edit_item' => sprintf(__('Bearbeite %s','wdf'), $single),
			'new_item' => sprintf(__('Neue %s','wdf'), $single),
			'all_items' => sprintf(__('Alle %s','wdf'), $plural),
			'view_item' => sprintf(__('%s ansehen','wdf'), $single),
			'search_items' => sprintf(__('Suche %s','wdf'), $plural),
			'not_found' =>  sprintf(__('Keine %s gefunden','wdf'), $plural),
			'not_found_in_trash' => sprintf(__('Keine %s im Papierkorb gefunden','wdf'), $plural),
			'parent_item_colon' => '',
			'menu_name' => $menu
		);
	...
	function create_donation_labels() {
		...
		$donation_labels = array(
			...
			'add_new' => sprintf(__('Neue %s','wdf'), $single),
			'add_new_item' => sprintf(__('Neue %s hinzufügen','wdf'), $single),
			'edit_item' => sprintf(__('%s bearbeiten','wdf'), $single),
			'new_item' => sprintf(__('Neue %s','wdf'), $single),
			'all_items' => sprintf(__('Alle %s','wdf'), $plural),
			'view_item' => sprintf(__('%s ansehen','wdf'), $single),
			'search_items' => sprintf(__('Suche %s','wdf'), $plural),
	...
	function _init() {
		...
		$approved_args = array(
			'label'       => __('Genehmigt', 'wdf'),
			'label_count' =>  _n_noop( 'Genehmigt <span class="count">(%s)</span>', 'Genehmigte <span class="count">(%s)</span>' ),
			'post_type'   => 'donation',
			'private'      => true,
		...
		$complete_args = array(
			'label'       => __('Komplett', 'wdf'),
			'label_count' =>  _n_noop( 'Komplett <span class="count">(%s)</span>', 'Komplette <span class="count">(%s)</span>' ),
			'post_type'   => 'donation',
			'private'      => true,
			'exclude_from_search' => true,
			'show_in_admin_status_list' => true,
		...
		$canceled_args = array(
			'label'       => __('Abgebrochen', 'wdf'),
			'label_count' =>  _n_noop( 'Abgebrochen <span class="count">(%s)</span>', 'Abgebrochene <span class="count">(%s)</span>' ),
			'post_type'   => 'donation',
			'private'      => true,
			'exclude_from_search' => true,
			'show_in_admin_status_list' => true,
		...
		$refunded_args = array(
			'label'       => __('Rückerstattung', 'wdf'),
			'label_count' =>  _n_noop( 'Rückerstattet <span class="count">(%s)</span>', 'Rückerstatte <span class="count">(%s)</span>' ),
			'post_type'   => 'donation',
			'private'      => true,
			'exclude_from_search' => true,
			'show_in_admin_status_list' => true,
			...
			if( $files = scandir($style_dir) ) {
				foreach ($files as $file) {
					...
					//$string = file_get_contents($style_dir.$style,null,null,null,100);
						switch($file) {
							case 'wdf-basic.css' :
								$styles['wdf-basic'] = __('Basis','wdf');
								break;
							case 'wdf-dark.css' :
								$styles['wdf-dark'] = __('Dunkel','wdf');
								break;
							case 'wdf-fresh.css' :
								$styles['wdf-fresh'] = __('Frisch','wdf');
								break;
	...
	function handle_payment() {
		...
		if($process_payment) {
			if( !isset($_SESSION['wdf_type']) || empty($_SESSION['wdf_type']) )
				$this->create_error(__('Der Unterstützungstyp konnte nicht bestimmt werden.','wdf'),'payment_submit');

			if( !isset($_SESSION['wdf_gateway']) || empty($_SESSION['wdf_gateway']) )
				$this->create_error(__('Bitte wähle eine Zahlungsmethode.','wdf'),'payment_submit');

			if( !isset($_SESSION['funder_id']) || empty($_SESSION['funder_id']) ) {
				$this->create_error(__('Spendenaktion konnte nicht ermittelt werden.','wdf'),'payment_submit');
			}
		...
		if(isset($this->is_funder_confirm) && $this->is_funder_confirm){
			...
			if($pledge_id)
				$this->create_error(__('Du hast noch keine Unterstützung abgegeben.','wdf'),'no_pledge');
	...
	function add_menu_meta_boxes() {
		$settings = get_option('wdf_settings');
		add_meta_box( 'add-funder', __('Spendenaktionen','wdf'), array(&$this,'meta_box_display'), 'nav-menus', 'side', 'default');
		?>
			<script type="text/javascript">
				jQuery(document).ready( function($) {
					$('#wdf_add_nav_archive').on('click', function() {
						<?php $funder_slug = $settings['dir_slug']; ?>
						wpNavMenu.addLinkToMenu('<?php echo trailingslashit(home_url($funder_slug)); ?>','Spendenaktionen');
						return false;
					});
				});
			</script>
		...
		if($typenow == 'funder') {
			...
			*/
			if($has_pledges == false)
				add_meta_box( 'wdf_type', sprintf(__('%s Typ','wdf'),esc_attr($settings['funder_labels']['menu_name'])), array(&$this,'meta_box_display'), 'funder', 'side', 'high');

			if( $has_pledges != false || $post->post_status == 'publish' )
				add_meta_box( 'wdf_progress', sprintf(__('%s Fortschritt','wdf'),esc_attr($settings['funder_labels']['singular_name'])), array(&$this,'meta_box_display'), 'funder', 'side', 'high');

			add_meta_box( 'wdf_options', sprintf(__('%s Einstellungen','wdf'),esc_attr($settings['funder_labels']['singular_name'])), array(&$this,'meta_box_display'), 'funder', 'side', 'high');
			add_meta_box( 'wdf_goals', sprintf(__('Lege Deine %s Ziele fest','wdf'),esc_attr($settings['funder_labels']['singular_name'])), array(&$this,'meta_box_display'), 'funder', 'normal', 'high');

			add_meta_box( 'wdf_messages', __('Danke Nachricht-Einstellungen','wdf'), array(&$this,'meta_box_display'), 'funder', 'normal', 'high');
			// Show pledge activity if funds have been raised
			if( $has_pledges != false )
				add_meta_box( 'wdf_activity', sprintf(__('%s Aktivität','wdf'), esc_attr($settings['donation_labels']['singular_name'])), array(&$this,'meta_box_display'), 'funder', 'normal', 'high');
	...
	function admin_menu() {
		...
		add_submenu_page( 'edit.php?post_type=funder', sprintf(__('%s Einstellungen','wdf'), $settings['funder_labels']['menu_name']), __('Einstellungen','wdf'), 'wdf_edit_settings', 'wdf_settings', array(&$this,'admin_display') );
		add_submenu_page( 'edit.php?post_type=funder', __('Getting Started','wdf'), __('Erste Schritte','wdf'), 'wdf_add_fundraisers', 'wdf', array(&$this,'admin_display') );
		...
		switch($_GET['page']) {
			case 'wdf_settings' :
				if(!current_user_can('wdf_edit_settings'))
					wp_die(__('Du hast keine Berechtigung diese Seite anzuzeigen.','wdf'));
	...
	function save_settings($new) {
		...
		if (!wp_verify_nonce($nonce,'_wdf_settings_nonce') ) {
			$this->create_error(__('Sicherheitsüberprüfung fehlgeschlagen. Was machst du?','wdf'), 'wdf_nonce');
			$die = true;
		}
		...
		foreach($new as $k => $v) {
			if($k == 'slug') {
				$new[$k] = sanitize_title($v,__('Spenden','wdf'));
		...
		if(!$die) {
			...
			update_option('wdf_settings',$settings);
			$this->create_msg(__('Einstellungen gespeichert','wdf'), 'general');
		}
	}
	...
	function create_error($msg, $context) {
		...
		add_filter('wdf_error_' . $context, function($content) {$content;});
		$this->wdf_error = true;
	}
	...
	function create_msg($msg, $context) {
		...
		add_filter('wdf_msg_' . $context, function() {$content;});
				...
				if($value != '') {
					if($key == 'goal') {
						...
						if($is_page == false && $is_post == false) {
							$this->create_error('Du musst eine gültige Post- oder Seiten-ID angeben','thanks_post');
							$value = '';
						}
					} else {
						$value = esc_attr($value);
		...
		if($typenow == 'funder' || $pagenow == 'admin.php') {
			...
			if( $hook === 'post.php' || $hook === 'post-new.php') {
				...
				$translation_array = array(
					'title_remind' => __( 'Bitte gib zuerst den Titel ein.','wdf')
				);
	...
	function edit_columns($columns) {
		...
		if($typenow == 'funder') {
			...
			$columns['funder_raised'] = __('Erhalten','wdf');
			$columns['title'] = $move_title;
			$columns['funder_type'] = __('Typ','wdf');
			$columns['funder_time_left'] = __('Übrige Zeit', 'wdf');
			$columns['comments'] = $move_comments;

			unset($columns['date']);
		} elseif($typenow == 'donation') {
			$columns['pledge_amount'] = __('Betrag', 'wdf');
			$columns['pledge_status'] = __('Status', 'wdf');
			$columns['pledge_recurring'] = __('Wiederholend', 'wdf');
			...
			$columns['pledge_method'] = __('Methode', 'wdf');
	...
	function column_display($name) {
		...
		switch($name) {
			...
			case 'pledge_amount' :
				...
				$reward = (isset($trans['reward'])) ? ' ('.__('Belohnung: ','wdf').$trans['reward'].')' : '';
	...
	function media_upload_tabs($tabs) {
		...
		if(isset($_GET['tab'])) {
			if($_GET['tab'] == 'fundraising' || $_GET['tab'] == 'donate_button' || $_GET['tab'] == 'progress_bar' || $_GET['tab'] == 'pledges') {
				...
				$tabs['fundraising'] = sprintf(__('%s Formular','wdf'),esc_attr($settings['funder_labels']['singular_name']));
				$tabs['pledges'] = sprintf(__('%s Liste','wdf'),esc_attr($settings['donation_labels']['plural_name']));
				$tabs['progress_bar'] = __('Fortschrittsanzeige','wdf');
			}
		}
	...
	function media_buttons($context) {
		...
		if($typenow != 'funder' && $typenow != 'donation' && $context == 'content' && $pagenow != 'index.php') {
			echo '<a title="'.__('Füge Fundraising-Shortcodes ein','wdf').'" class="thickbox add_media" id="add_wdf" href="'.admin_url('media-upload.php?post_id='.$post->ID).'&tab=donate_button&TB_iframe=1&wdf=1"><img onclick="return false;" alt="'.__('Füge Fundraising-Shortcodes ein','wdf').'" src="'.WDF_PLUGIN_URL.'/img/sm_ico.png"></a>';
		}
	}
	function media_progress_bar_iframe() {
		...
		<form class="wdf_media_cont media-item" id="media_progress_bar">
			<h3 class="media-title"><?php _e('Füge einen Fortschrittsbalken hinzu','wdf'); ?></h3>
			<span class="description"><?php _e('Nur Spendenaktionen, die ein Ziel haben, können einen Fortschrittsbalken anzeigen','wdf'); ?></span>
			<div id="media-items">
						...
						<tbody>
							<tr>
								<th valign="top" scope="row" class="label">
									<span class="alignleft"><?php echo sprintf(__('Wähle ein %s','wdf'),esc_attr($settings['funder_labels']['singular_name'])); ?></span>
								</th>
								<td>
									<?php if(!$funders || empty($funders)) : ?>
										<div class="message alert"><?php echo sprintf(__('Du hast noch keine %s erstellt.','wdf'),esc_attr($settings['funder_labels']['plural_name'])); ?></div>
									<?php else : ?>
							...
							<tr>
								<th valign="top" scope="row" class="label"><span class="alignleft"><label><?php _e('Titel anzeigen','wdf'); ?></label></span></th>
								<td><input type="checkbox" name="show_title" value="yes" /></td>
							</tr>
							<tr>
								<th valign="top" scope="row" class="label"><span class="alignleft"><label><?php _e('Summen anzeigen','wdf'); ?></label></span></th>
								<td><input type="checkbox" name="show_totals" value="yes" /></td>
							</tr>
							<tr>
								<th valign="top" scope="row" class="label"><span class="alignleft"><label><?php _e('Wähle einen Stil','wdf'); ?></label></span></th>
								<td>
									<select name="style">
										<?php if(is_array($this->styles) && !empty($this->styles)) : ?>
											<option value="0"><?php _e('Standard','wdf'); ?></option>
							...
							<tr>
								...
								<td><p class="alignright"><a class="button" href="#" id="wdf_add_shortcode" onclick="window.parent.wdf_inject_shortcode(); return false;"><?php _e('Shortcode einfügen','wdf'); ?></a></p></td>
							</tr>
						</tbody>
					</table>
				</div>
	...
	function media_donate_button_iframe () {
		...
		<form class="wdf_media_cont" id="media_donate_button">
			<h3 class="media-title"><?php _e('Füge einen Spendenknopf hinzu','wdf'); ?></h3>
			<div id="media-items">
				<div class="media-item media-blank">
					<p class="media-types">
						<?php /*?><label class="description"><?php _e('Button Type','wdf'); ?></label><br/><?php */?>
						<label><input onChange="window.parent.wdf_input_switch(this); return false;" class="wdf_toggle" type="radio" name="button_type" value="default" rel="wdf_button_type"/> <?php _e('Standard PayPal Schaltfläche','wdf'); ?></label>
						<label><input onChange="window.parent.wdf_input_switch(this); return false;" class="wdf_toggle" type="radio" name="button_type" value="custom" rel="wdf_button_type"/> <?php _e('Benutzerdefinierte Schaltfläche','wdf');?></label>
					</p>
					<table class="describe">
						<tbody>
							<tr>
								<th valign="top" scope="row" class="label">
									<span class="alignleft"><label><?php _e('Titel','wdf') ?></label></span>
							...
							<tr>
								<th valign="top" scope="row" class="label">
									<span class="alignleft"><label><?php _e('Beschreibung','wdf') ?></label></span>
								</th>
								<td class="field"><textarea name="description"></textarea></td>
							</tr>
							<tr>
								<th valign="top" scope="row" class="label">
									<span class="alignleft"><label><?php echo sprintf(__('%s Betrag','wdf'),esc_attr($settings['donation_labels']['singular_name'])); ?><br /><span class="description"><?php _e('(blank = choice)','wdf'); ?></span></label></span>
								</th>
								<td class="field"><input type="text" name="donation_amount" value="" /></td>
							</tr>
							<tr class="wdf_button_type" rel="custom">
								<th valign="top" scope="row" class="label">
									<span class="alignleft"><label><?php _e('Wähle einen Stil','wdf') ?></label></span>
								</th>
								<td class="field">
									<select name="style">
										<?php if(is_array($this->styles) && !empty($this->styles)) : ?>
							...
							<tr class="wdf_button_type" rel="custom">
								<th valign="top" scope="row" class="label">
									<span class="alignleft"><label><?php _e('Schaltflächentext','wdf'); ?></label></span>
								</th>
								<td class="field"><input type="text" name="button_text" value="" /></td>
							</tr>
							<tr class="wdf_button_type" rel="default">
								<th valign="top" scope="row" class="label">
									<span class="alignleft"><label><?php _e('Paypal Optionen','wdf'); ?></label></span>
								</th>
								<td class="field">
									<p><label><input type="checkbox" name="show_cc" value="yes" /> <?php _e('Akzeptierte Kreditkarten anzeigen','wdf'); ?></label></p>
									<p><label><input type="checkbox" name="allow_note" value="yes" /> <?php _e('Spezielles Anweisungsfeld zulassen','wdf'); ?></label></p>
									<p><label><input type="checkbox" name="small_button" value="yes" /> <?php _e('Kleine Schaltfläche verwenden','wdf'); ?></label></p>
								</td>
							</tr>
							<tr>
								<th valign="top" scope="row" class="label">
									<span class="alignleft"><label><?php _e('PayPal Email','wdf') ?><br /><span class="description"><?php _e('Standardmäßig werden die Einstellungen verwendet','wdf'); ?></span></label></span>
								</th>
								<td class="field">
	...
	function media_fundraiser_iframe() {
			...
			<h3 class="media-title"><?php printf(__('Ein %s Formular hinzufügen','wdf'), $settings['funder_labels']['singular_name']); ?></h3>
				...
				<div class="media-item media-blank">
					<table class="describe">
						<tbody>
							...
							<tr>
								<th valign="top" scope="row" class="label"><span class="alignleft"><label><?php _e('Inhalt zeigen','wdf'); ?></label></span></th>
	...
	function media_pledges_iframe() {
			...
			<h3 class="media-title"><?php printf(__('Liste von %s hinzufügen','wdf'), $settings['donation_labels']['plural_name']); ?></h3>
						...
						<tbody>
							<tr>
								...
								<td>
									...
									<?php else : ?>
									<select id="wdf_funder_select" name="id">
										<option value="0"><?php printf(__('Alle %s','wdf'), $settings['funder_labels']['plural_name']); ?></option>
											<?php foreach($funders as $funder) { ?>
												<option value="<?php echo $funder->ID; ?>"><?php echo $funder->post_title ?></option>
											<?php } ?>
									</select>
							...
							<tr>
								<th valign="top" scope="row" class="label"><span class="alignleft"><label><?php _e('Sortiertyp','wdf'); ?></label></span></th>
								<td>
									<select name="sort_type">
										<option value="last"><?php _e('Neueste zuerst','wdf'); ?></option>
										<option value="top"><?php _e('Größte zuerst','wdf'); ?></option>
									</select>
								</td>
							</tr>
							<tr>
								<th valign="top" scope="row" class="label"><span class="alignleft"><label><?php printf(__('Anzahl der anzuzeigenden %s','wdf'),esc_attr($settings['donation_labels']['singular_name']) ); ?></label></span></th>
    ...
    function format_cycle( $cycle ){
        ...
        switch($cycle){
            case 'D':
            case '1 D':
                $period = __( 'Täglich', 'wdf' );
                break;
            case 'W':
            case '1 W':
                $period = __( 'Wöchentlich', 'wdf' );
                break;
            case 'M':
            case '1 M':
                $period = __( 'Monatlich', 'wdf' );
                break;
            case 'Y':
            case '1 Y':
                $period = __( 'Jährlich', 'wdf' );
                break;
        }
    ...
    function get_recurring_column_detail( $transaction ){
        if(!isset($transaction['cycle'])){
            ...
            if( isset( $transaction['recurring_transactions'] ) ){
                $recurring_transactions = $transaction['recurring_transactions'] . ' ' . _n( 'Bezahlvorgang abgeschlossen', 'Bezahlvorgänge abgeschlossen', $transaction['recurring_transactions'], 'wdf' );
